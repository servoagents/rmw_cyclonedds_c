...
endpoint state on the standard ROS graph topic and maintains a bounded remote
cache for node names, topic names/types, and endpoint counts. It rejects
strings and variable-size application sequences, content filters, loaned and
serialized messages, services, clients, events, extended graph queries, and
DDS Security.
