...
docker run --rm --network none \
  ...
  "${IMAGE}" \
  bash -lc ". /opt/ros/${ROS_DISTRO}/setup.sh && . /workspace/install/setup.sh && exec /workspace/install/rmw_cyclonedds_c/lib/rmw_cyclonedds_c/contract_test" \
  >"${OUTPUT_DIR}/stdout.log" \
  2>"${OUTPUT_DIR}/stderr.log"

grep -q '^RMW_CYCLONEDDS_C_CONTRACT_PASS keep_all=rejected zero_depth=rejected default_durability=rejected type=rejected timeout=passed guard=passed$' \
  "${OUTPUT_DIR}/stdout.log"
