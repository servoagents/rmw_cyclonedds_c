...
static rmw_ret_t map_dds_result(dds_return_t result, const char * operation)
{
  ...
  RMW_SET_ERROR_MSG_WITH_FORMAT_STRING(
    "%s failed: %s (%" PRId32 ")", operation, dds_strretcode(-result), result);
...
static bool qos_is_supported(const rmw_qos_profile_t * qos)
{
  ...
  {
    RMW_SET_ERROR_MSG(
      "Phase 3 supports ROS naming and best-effort/volatile/keep-last QoS only");
    return false;
  }
  return true;
}
  ...
  rmw_init_options_t * init_options, rcutils_allocator_t allocator)
{
  ...
  rmw_ret_t result = rmw_discovery_options_init(
    &initialized.discovery_options, 0U, &initialized.allocator);
...
rmw_ret_t rmw_init_options_copy(
  const rmw_init_options_t * source, rmw_init_options_t * destination)
{
  ...
  if (result == RMW_RET_OK) {
    result = rmw_discovery_options_copy(
      &source->discovery_options, &copy.allocator, &copy.discovery_options);
  }
  if (result == RMW_RET_OK) {
    result = rmw_security_options_copy(
      &source->security_options, &copy.allocator, &copy.security_options);
  }
...
rmw_ret_t rmw_init(const rmw_init_options_t * options, rmw_context_t * context)
{
  ...
  implementation->participant = dds_create_participant(
    (dds_domainid_t)domain_id, NULL, NULL);
  ...
  implementation->graph_guard_entity =
    dds_create_guardcondition(implementation->participant);
...
rmw_ret_t rmw_shutdown(rmw_context_t * context)
{
  ...
  return map_dds_result(
    dds_set_guardcondition(context->impl->graph_guard_entity, true),
...
rmw_ret_t rmw_context_fini(rmw_context_t * context)
{
  ...
  rmw_ret_t result = map_dds_result(
    dds_delete(implementation->participant), "dds_delete participant");
  const rmw_ret_t domain_result = map_dds_result(
    dds_delete(implementation->domain), "dds_delete domain");
...
rmw_guard_condition_t * rmw_create_guard_condition(rmw_context_t * context)
{
  ...
  rmw_guard_condition_t * guard_condition =
    allocate_zeroed(allocator, sizeof(*guard_condition));
...
{
  if (guard_condition == NULL ||
    !identifiers_match(guard_condition->implementation_identifier) ||
  ...
  const rmw_ret_t result = map_dds_result(
    dds_delete(data->entity), "dds_delete guard condition");
  allocator.deallocate(data, allocator.state);
  allocator.deallocate(guard_condition, allocator.state);
  return result;
}

static rmw_publisher_t * create_publisher_handle(
  rmw_context_impl_t * context, const char * topic_name,
...
rmw_ret_t rmw_get_gid_for_publisher(const rmw_publisher_t * publisher, rmw_gid_t * gid)
{
  ...
  const size_t handle_size = sizeof(handle) < RMW_GID_STORAGE_SIZE ?
    sizeof(handle) : RMW_GID_STORAGE_SIZE;
...
rmw_ret_t rmw_destroy_subscription(rmw_node_t * node, rmw_subscription_t * subscription)
{
  ...
  const rmw_ret_t reader_result = map_dds_result(
    dds_delete(endpoint->endpoint), "dds_delete reader");
  const rmw_ret_t topic_result = map_dds_result(dds_delete(endpoint->topic), "dds_delete topic");
  ...
  rmw_message_info_t * message_info, rmw_subscription_allocation_t * allocation)
{
  ...
  *message_info = rmw_get_zero_initialized_message_info();
  ...
  const size_t handle_size = sizeof(sample_info.publication_handle) < RMW_GID_STORAGE_SIZE ?
    sizeof(sample_info.publication_handle) : RMW_GID_STORAGE_SIZE;
...
rmw_wait_set_t * rmw_create_wait_set(rmw_context_t * context, size_t max_conditions)
{
  ...
  if (capacity > SIZE_MAX / sizeof(dds_entity_t) ||
    capacity > SIZE_MAX / sizeof(dds_attach_t))
  {
...
static bool attachment_was_triggered(
  const wait_set_data_t * data, dds_entity_t attachment, size_t triggered_count)
{
  ...
  rmw_wait_set_t * wait_set, const rmw_time_t * wait_timeout)
{
  ...
  {
    RMW_SET_ERROR_MSG("services, clients, and events are outside the Phase 3 profile");
    return RMW_RET_UNSUPPORTED;
  }
  ...
  const dds_return_t wait_result = dds_waitset_wait(
    data->waitset, data->triggered, data->capacity, timeout);
  if (wait_result < 0) {
    return map_dds_result(wait_result, "dds_waitset_wait");
  }
  const size_t triggered_count = (size_t)wait_result < data->capacity ?
    (size_t)wait_result : data->capacity;
