...
{
  fprintf(stderr, "PHASE3_CONTRACT_FAIL operation=%s rcl_ret=%d\n", operation, result);
  return 1;
}

static int expect_creation_failure(
  rcl_node_t * node,
  ...
  rmw_qos_profile_t qos)
{
  ...
  options.qos = qos;
  const rcl_ret_t result = rcl_publisher_init(
    &publisher, node, type_support, topic, &options);
...
int main(void)
{
  ...
  rcl_ret_t call_result = rclc_node_init_with_options(
    &node, "phase3_contract_negative", "", &support, &node_options);
  ...
  if (expect_creation_failure(
      &node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, UInt32),
      "phase3_reliable_rejected", reliable_qos) != 0)
  {
    goto cleanup_node;
  }
  if (expect_creation_failure(
      &node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32),
      "phase3_type_rejected", rmw_qos_profile_sensor_data) != 0)
  {
    goto cleanup_node;
  }

  call_result = rcl_guard_condition_init(
    &guard_condition, &support.context, rcl_guard_condition_get_default_options());
  if (call_result != RCL_RET_OK) {
    result = fail("guard_condition_init", call_result);
    goto cleanup_node;
  }
  call_result = rcl_wait_set_init(
    &wait_set, 0U, 1U, 0U, 0U, 0U, 0U, &support.context, allocator);
  if (call_result != RCL_RET_OK) {
  ...
  printf("PHASE3_CONTRACT_PASS reliable=rejected type=rejected timeout=passed guard=passed\n");
  result = 0;
