...
enum
{
  RMW_TO_ROS_VALUE = 271828182U,
  ROS_TO_RMW_VALUE = 314159265U,
  MAX_WAITS = 300
};
...
static int run_publisher(rcl_node_t * node)
{
  ...
  if (rclc_publisher_init_best_effort(
      ...
      "phase3_rmw_to_ros") != RCL_RET_OK)
  {
    fprintf(stderr, "PHASE3_ERROR operation=publisher_init\n");
    return result;
  }
  ...
  for (unsigned int wait = 0U; wait < MAX_WAITS && matched == 0U; ++wait) {
    if (rcl_publisher_get_subscription_count(&publisher, &matched) != RCL_RET_OK ||
      !pause_100_ms())
    {
      fprintf(stderr, "PHASE3_ERROR operation=wait_for_subscription\n");
      goto cleanup;
    }
  }
  if (matched == 0U) {
    fprintf(stderr, "PHASE3_TIMEOUT direction=rmw_to_ros stage=match\n");
    goto cleanup;
  }
  ...
  for (unsigned int sequence = 1U; sequence <= 5U; ++sequence) {
    if (rcl_publish(&publisher, &message, NULL) != RCL_RET_OK) {
      fprintf(stderr, "PHASE3_ERROR operation=publish\n");
      goto cleanup;
    }
    ...
    fflush(stdout);
    if (!pause_100_ms()) {
      fprintf(stderr, "PHASE3_ERROR operation=publish_delay\n");
      goto cleanup;
    }
  }
  result = 0;
  ...
  rcl_node_t * node, rclc_support_t * support, rcl_allocator_t * allocator)
{
  ...
  if (rclc_subscription_init_best_effort(
      &subscription, node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, UInt32),
      "phase3_ros_to_rmw") != RCL_RET_OK)
  {
    fprintf(stderr, "PHASE3_ERROR operation=subscription_init\n");
    return result;
  }
  if (rclc_executor_init(&executor, &support->context, 1U, allocator) != RCL_RET_OK) {
    fprintf(stderr, "PHASE3_ERROR operation=executor_init\n");
    goto cleanup_subscription;
  }
  if (rclc_executor_add_subscription(
      &executor, &subscription, &message, receive_uint32, ON_NEW_DATA) != RCL_RET_OK)
  {
    fprintf(stderr, "PHASE3_ERROR operation=executor_add_subscription\n");
  ...
  for (unsigned int wait = 0U; wait < MAX_WAITS && !received; ++wait) {
    ...
    if (spin_result != RCL_RET_OK && spin_result != RCL_RET_TIMEOUT) {
      fprintf(stderr, "PHASE3_ERROR operation=spin_some rcl_ret=%d\n", spin_result);
      goto cleanup_executor;
    }
  }
  if (received && received_value == ROS_TO_RMW_VALUE) {
    printf("RMW_RECEIVED direction=ros_to_rmw value=%u expected=%u\n",
      received_value, ROS_TO_RMW_VALUE);
    result = 0;
  } else {
    fprintf(stderr, "PHASE3_TIMEOUT direction=ros_to_rmw value=%u\n", received_value);
  }
...
int main(int argc, char ** argv)
{
  ...
  if (rclc_support_init(&support, 0, NULL, &allocator) != RCL_RET_OK) {
    fprintf(stderr, "PHASE3_ERROR operation=support_init\n");
    return result;
  }
  rcl_node_options_t node_options = rcl_node_get_default_options();
  node_options.enable_rosout = false;
  if (rclc_node_init_with_options(
      &node, "phase3_rmw_interop", "", &support, &node_options) != RCL_RET_OK)
  {
    fprintf(stderr, "PHASE3_ERROR operation=node_init\n");
